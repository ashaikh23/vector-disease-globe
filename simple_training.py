#!/usr/bin/env python3
"""
Simple Local Training Pipeline
Trains disease risk prediction model using RandomForest (no XGBoost dependency issues)
"""
...
class SimpleDiseaseRiskTrainer:
    """Simple training pipeline for disease risk prediction"""
    ...
    def train_models(self):
        """Train RandomForest models for each disease"""
    ...
    def generate_global_predictions(self):
        ...
        # Create global grid (smaller for demo)
        lats = np.arange(-60, 60, 2.0)  # Reduced resolution for faster processing
        ...
        print("\nRisk statistics:")
        for disease in ['dengue', 'malaria', 'lyme']:
            if f'{disease}_risk' in predictions_df.columns:
                risk_col = f'{disease}_risk'
                print(f"{disease}: min={predictions_df[risk_col].min():.3f}, max={predictions_df[risk_col].max():.3f}, mean={predictions_df[risk_col].mean():.3f}")
        
        return predictions_df
    
    def run_full_pipeline(self):
        """Run the complete training pipeline"""
        print("Starting Simple Disease Risk Training Pipeline...")
...
def main():
    ...
    trainer = SimpleDiseaseRiskTrainer()
